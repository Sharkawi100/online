...
/**
 * Escape output for HTML (shorter version of htmlspecialchars)
 ...
 */
function requireRoles($roles, $redirectTo = '/auth/login.php')
{
    if (!isLoggedIn()) {
        redirect($redirectTo);
        exit();
    ...
    if (!hasAnyRole($roles)) {
        $_SESSION['error'] = 'ليس لديك الصلاحية للوصول لهذه الصفحة';
        redirect($redirectTo);
        exit();
    }
}
 ...
 * @param string $requiredRole The role required for non-admins
 * @return bool
 */
